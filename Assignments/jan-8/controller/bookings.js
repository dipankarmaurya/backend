...
function updateSheet(bookings) {
    fs.writeFile('../jan-8//model/bookings.js', "module.exports = " + JSON.stringify(bookings), (err) => {
        if (err) {
            console.log(err)
            return { "error": err }
        } else {
...
function makeUpdatesInFiles(booking, show) {
    ...
    fs.writeFile("../jan-8/model/bookings.js", "module.exports = "+JSON.stringify(shows), (err) =>{
        if (err) {
            console.log(err)
            return {"error": err}
        } else {
...
module.exports = {
    ...
    deleteBooking: function deleteBooking(booking_id) {
        ...
        if (bookings.some((booking) => booking.id == booking_id)) {
            let newBookings = bookings.filter((booking) => booking.id != booking_id);
            updateSheet(newBookings);
        }
        // delete the booking
